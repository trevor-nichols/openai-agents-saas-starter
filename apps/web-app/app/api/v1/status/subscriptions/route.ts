...
import { createStatusSubscription, listStatusSubscriptions, StatusSubscriptionServiceError } from '@/lib/server/services/statusSubscriptions';
...
export async function POST(request: Request) {
  try {
    ...
    const authMode =
      typeof payload === 'object' && payload !== null && (payload as { channel?: string }).channel === 'webhook'
        ? 'session'
        : 'public';
    const subscription = await createStatusSubscription(payload as never, { authMode });
