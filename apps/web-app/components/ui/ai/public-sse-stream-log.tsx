...
type EventWithOptionalReceivedAt = PublicSseEvent & { receivedAt?: string };

export interface PublicSseStreamLogProps {
  events: EventWithOptionalReceivedAt[];
  className?: string;
}

type KnownKind = NonNullable<PublicSseEvent['kind']>;
...
function mapTool(tool: Extract<PublicSseEvent, { kind: 'tool.status' }>['tool']): ToolViewModel {
...
export function PublicSseStreamLog({ events, className }: PublicSseStreamLogProps) {
  const grouped = useMemo(() => events, [events]);

  if (grouped.length === 0) {
    return <p className={cn('text-sm text-foreground/60', className)}>No events yet.</p>;
  }
            ...
            {(() => {
              switch (evt.kind) {
                ...
                case 'memory.checkpoint':
                  return (
                    <div className="mt-3">
                      <CodeBlock code={JSON.stringify(evt.checkpoint, null, 2)} language="json" />
                    </div>
                  );
                case 'message.delta':
                  return (
                ...
                case 'chunk.done':
                  return (
                    <div className="mt-3">
                      <CodeBlock code={JSON.stringify({ target: evt.target }, null, 2)} language="json" />
                    </div>
                  );
                case 'error':
                  return (
