...
- `NEXT_PUBLIC_API_URL` (or `API_BASE_URL`) points the BFF to FastAPI (default `http://localhost:8000`).
- `NEXT_PUBLIC_ENABLE_BILLING` gates billing routes and UI.
- `AGENT_API_MOCK=true` (server) and `NEXT_PUBLIC_AGENT_API_MOCK=true` (client) enable mock data flows for local development.
