"""Shared mapping for OpenAI Agents SDK stream events."""
...
def map_stream_event(
    ...
    *,
    ...
    if event_type == "raw_response_event":
        return map_raw_response_event(event, response_id=response_id, metadata=metadata)
    if event_type == "run_item_stream_event":
        return map_run_item_event(event, response_id=response_id, metadata=metadata)
...
def map_raw_response_event(
    ...
    *,
    ...
    if annotations is None and isinstance(raw_mapping, Mapping):
        ...
        if isinstance(inline_annotations, list):
            filtered = [
                ...
                and ann.get("type") in {"url_citation", "container_file_citation", "file_citation"}
            ]
            if filtered:
    ...
    container_mode = metadata.get("code_interpreter_mode") if metadata else None

    tool_call = _maybe_build_tool_call_from_raw(
        raw_type=raw_type,
...
def map_run_item_event(
    ...
    *,
    ...
    tool_call = _maybe_build_tool_call_from_item(
        item=item,
        annotations=annotations,
        container_mode=container_mode,
    )
