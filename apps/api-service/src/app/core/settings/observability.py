...
class ObservabilitySettingsMixin(BaseModel):
    ...
    logging_file_path: str = Field(
        ...
        description="Filesystem path for file sink when LOGGING_SINKS includes file.",
        alias="LOGGING_FILE_PATH",
    )
    logging_file_max_mb: int = Field(
        default=10,
        ge=1,
        description="Maximum megabytes per log file when LOGGING_SINKS includes file.",
        alias="LOGGING_FILE_MAX_MB",
    )
    logging_file_backups: int = Field(
        default=5,
        ge=0,
        description="Rotated backup files to retain when LOGGING_SINKS includes file.",
    ...
    logging_duplex_error_file: bool = Field(
        default=False,
        description=(
            "When LOGGING_SINKS includes stdout, also write errors to the dated error file."
        ),
    ...
    logging_datadog_api_key: str | None = Field(
        default=None,
        description="Datadog API key when LOGGING_SINKS includes datadog.",
    ...
    logging_otlp_endpoint: str | None = Field(
        default=None,
        description="OTLP/HTTP endpoint when LOGGING_SINKS includes otlp.",
        alias="LOGGING_OTLP_ENDPOINT",
    )
    logging_otlp_headers: str | None = Field(
        default=None,
        description="Optional OTLP headers JSON when LOGGING_SINKS includes otlp.",
