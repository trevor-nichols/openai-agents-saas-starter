...
| --- | --- | --- | --- |
| `/api/v1/auth` | `test_auth_smoke.py`, `test_auth_sessions_smoke.py`, `test_auth_email_password_smoke.py`, `test_auth_mfa_smoke.py`, `test_auth_signup_smoke.py`, `test_auth_sso_smoke.py`, `test_service_accounts_smoke.py` | Login/refresh/me, session management, signup/invites/requests/register, email verification + password flows, MFA TOTP, SSO provider list + start, service-account issuance + token admin | Gates: `SMOKE_ENABLE_AUTH_SIGNUP`, `SMOKE_ENABLE_AUTH_EXTENDED`, `SMOKE_ENABLE_AUTH_MFA`, `SMOKE_ENABLE_AUTH_SSO`, `SMOKE_ENABLE_SERVICE_ACCOUNTS`; fixtures seed operator/unverified/MFA users + password-reset token endpoint. |
...
| `/api/v1/features` | `test_features_smoke.py` | Feature snapshot returns `billing_enabled` + `billing_stream_enabled` | Requires tenant context. |
...
| `/api/v1/platform` | `test_platform_smoke.py` | Operator tenant list + detail lookups + feature entitlements | Requires `platform:operator` token (fixture seeds a platform operator user). |
...
| `/api/v1/billing` | `test_billing_smoke.py` | Plans list + subscription lifecycle (start/update/cancel) + portal session + setup intent + payment methods list + upcoming invoice preview + optional plan change + usage record + usage totals list + events list + stream handshake | `SMOKE_ENABLE_BILLING=1`; stream requires `SMOKE_ENABLE_BILLING_STREAM=1`. |
