...
| `/api/v1/billing` | `test_billing_smoke.py` | Plans list + subscription lifecycle (start/update/cancel) + portal session + setup intent + payment methods list + upcoming invoice preview + optional plan change + usage record + events list + stream handshake | `SMOKE_ENABLE_BILLING=1`; stream requires `SMOKE_ENABLE_BILLING_STREAM=1`. |
