...
- FastAPI + worker entrypoints call `app.observability.logging.configure_logging(get_settings())`, which wires the JSON formatter and sinks declared via `LOGGING_SINKS` (comma-separated) or the legacy `LOGGING_SINK` (single). Supported sinks: `stdout`, `file`, `datadog`, `otlp`, `none` (mutually exclusive). When the Starter CLI flips `ENABLE_OTEL_COLLECTOR=true`, `LOGGING_OTLP_ENDPOINT` defaults to `http://otel-collector:4318/v1/logs`, so the app streams logs into the bundled OpenTelemetry Collector automatically. Set `LOGGING_DATADOG_API_KEY`/`LOGGING_DATADOG_SITE` or `LOGGING_OTLP_ENDPOINT` (+ optional `LOGGING_OTLP_HEADERS` JSON) before switching sinks; startup fails fast when configuration is incomplete.
