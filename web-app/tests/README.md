...
- **Archived conversation reset**: Flow 8 temporarily deletes the archive seed and then calls `POST /api/v1/test-fixtures/apply` to restore it. If `USE_TEST_FIXTURES` is disabled the replay step will fail and subsequent runs will permanently lose that transcript.
