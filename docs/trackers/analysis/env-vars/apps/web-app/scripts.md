...
| **`NEXT_PUBLIC_API_URL`** | Fallback API URL used for fixture seeding if `PLAYWRIGHT_API_URL` is not set. | `test-seed.js` (Line 99) | Optional | `http://localhost:8000` | URL string | Non-secret config |
| **`PLAYWRIGHT_API_URL`** | Primary override for the backend API URL when running seed fixtures. | `test-seed.js` (Line 98) | Optional | `NEXT_PUBLIC_API_URL` or `http://localhost:8000` | URL string | Non-secret config |
