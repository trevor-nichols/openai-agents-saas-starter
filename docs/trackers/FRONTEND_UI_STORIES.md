...
| UI-008 | Marketing `/status` | Build status cards, uptime stats, incident log table, and alert CTA; reuse `StatCard`, `InlineTag`, `Table`. | `StatCard`, `Table`, `GlassPanel`, `InlineTag` | Done | @platform | JSON + RSS feeds now proxy live FastAPI status data through the BFF + TanStack hook, and the route consumes the new `StatusExperience` feature module. |
