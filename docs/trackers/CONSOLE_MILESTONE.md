...
**Status:** In Progress  
**Goal:** Make the Textual hub the authoritative UI surface while keeping console subcommands as automation-mode entrypoints, with clean UI boundaries and a shared hub data model.
...
- UI no longer imports console command modules; shared logic lives in services/workflows.
- `__init__.py` no longer mutates `sys.path`.
- Console snapshot + docs updated; tests added/adjusted for ports/presenters/hub.
- `cd packages/starter_console && hatch run lint && hatch run typecheck` pass.

#### Remediation Plan (Post-Review)
- Replace Setup Hub subprocess actions with routed TUI actions (no nested console runs).
...
| B3 | Workflow Integration | TUI + console subcommands use shared workflows. | @platform-foundations | ⏳ |
...
| D4 | Snapshot + Tests | Refresh console snapshot + add tests for ports/presenters/UI routing. | @platform-foundations | ✅ |

---

## Completed Milestones

### Textual-First Console Hub (2025-12-23)
...
- Tracker: `docs/trackers/current_milestone/MILESTONE_CONSOLE_TEXTUAL_REDESIGN.md`.
