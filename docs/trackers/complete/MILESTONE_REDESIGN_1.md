...
## Vision
Deliver a clean implementation of the redesigned Starter CLI wizard so operators can complete all foundational setup tasks (infra, secrets, providers, env files) in one guided pass, leaving only `cd api-service && hatch run serve` and `pnpm dev` afterward.
