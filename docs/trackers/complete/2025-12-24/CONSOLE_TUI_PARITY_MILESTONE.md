<!-- SECTION: Metadata -->
# Milestone: Console ↔ TUI Parity & IA Redesign

_Last updated: 2025-12-24_  
**Status:** Complete  
**Owner:** Platform Foundations  
**Domain:** Console  
**ID / Links:** [Parity checklist](docs/trackers/checklist/CONSOLE_TUI_PARITY.md), [Console snapshot](packages/starter_console/SNAPSHOT.md)
...
Deliver a professional, fully featured Textual TUI that reaches functional parity with the Starter Console, using a clean navigation structure, consistent interaction patterns, and a global context panel for env controls. Outcome: operators can complete all console workflows from the TUI with the same fidelity and auditability.
...
- All console commands/subcommands have a TUI surface or are explicitly documented as out-of-scope
...
- Existing panes updated to match console knobs and statuses (wizard, setup hub, logs, infra, providers, stripe)
- New panes added for missing console workflows (doctor, start/stop, release, config, api export, auth, usage sync/export, stripe dispatches, status ops, util)
- Shared UI patterns established (action form + output/log + status table) without duplicating domain logic
- Tests and checks pass (console + repo standards)
...
- Parity expansion for all console commands/subcommands
...
- Enforced service boundary: panes call service/workflow modules, not console command handlers.
...
- OpenAPI export runs via an api-service script to avoid the console importing backend modules directly.
- Logs “Follow” now uses streaming (`-f`) behavior to match console parity (no polling divergence).

New/updated modules (planned):
- `starter_console/ui/layout` (collapsible nav + global context panel)
- `starter_console/ui/panes/*` expansions and new panes for missing console workflows
...
| ID | Area | Description | Owner | Status |
|----|------|-------------|-------|--------|
| C1 | Wizard | Add missing console flags (report-only, export paths, automation overrides) | PF | ✅ |
...
| H2 | Tests | Add/expand console/TUI tests where needed | PF | ✅ |
...
- Remove remaining outdated compatibility references now that parity is complete:
  - Presenter console adapter now lives at `starter_console/presenters/console_adapter.py`
  - Audit docs/commands for any other outdated compatibility references and delete them
- Repo testing standards (console tests + lint/typecheck)
...
| Duplicated logic between console/TUI | Med | Route all actions through existing workflows/services |
...
- `cd packages/starter_console && hatch run lint && hatch run typecheck && hatch run test`
...
- Global Context panel reuses console env behavior; document in release notes
