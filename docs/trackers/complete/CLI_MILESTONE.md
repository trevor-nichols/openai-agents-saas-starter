...
| M5 – Package Extraction | Split the CLI into adapters/commands/services/workflows, add the new composition root, migrate wizard/secrets workflows into `starter_cli/workflows`, and co-locate CLI tests under `starter_cli/tests`. | New `app.py` + `container.py`, adapters/core/services modules, refreshed `SNAPSHOT.md`, updated docs + trackers, tests relocated from `anything-agents/tests`. | Platform Foundations | ✅ Complete (2025-11-18) | 2025-12-05 |
