...
2. **STRIPE-02 – Developer Setup Script**: Deliver the consolidated CLI command (`python -m starter_cli.cli stripe setup`, aliased by `pnpm stripe:setup`) so new contributors can authenticate the Stripe CLI, launch Postgres/Redis, auto-provision Starter/Pro prices, and persist secrets into `.env.local` safely.
