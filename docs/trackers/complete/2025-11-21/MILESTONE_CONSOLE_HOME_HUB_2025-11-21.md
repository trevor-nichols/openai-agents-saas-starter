...
## Success Criteria
- `starter-console home` renders a navigable hub or a concise summary with `--no-tui`.
- `starter-console doctor --json` exits non-zero on critical failures and emits machine-readable results.
- `starter-console start dev --open-browser` reliably brings up compose + backend + frontend and reports health within a bounded timeout.
...
| - | ---- | ----- | ------ | ------ |
| 1 | Define status models (`ProbeResult`, `ServiceStatus`, `ActionShortcut`, `LaunchPlan`) in `starter_console/core/status_models.py`. | Platform Foundations | ✅ Completed | Dec 2025 |
| 2 | Add probe library under `starter_console/workflows/home/probes/` (db, redis, api, frontend, ports, env coverage, migrations, stripe, vault). | Platform Foundations | ✅ Completed | Dec 2025 |
...
## Validation Log
- 2025-11-21: Unit tests `python -m pytest starter_console/tests/unit/home` ✅
...
| 2 – Doctor Command | Implement probes and doctor assembler with JSON/Markdown emitters and strict mode. | `starter-console doctor` useful; exits non-zero on errors; writes reports. | ✅ Completed (2025-11-21) |
...
| 6 | Quality gates | Record runs: `hatch run lint`, `hatch run typecheck`, `pytest starter_console/tests/unit/home`; optional smoke script for doctor without network; update tracker log with dates. | All gates green; tracker log updated; smoke script side-effect free. | ✅ Completed (2025-11-21) | Dec 2025 |

## Phase 0 Decisions
- **UI stack**: Rich remains baseline; Textual added as optional extra (`starter_console[tui]`) for the hub. Fallback to plain console with `--no-tui`.
