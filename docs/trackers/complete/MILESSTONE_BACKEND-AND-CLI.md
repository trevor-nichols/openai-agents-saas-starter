...
- **Starter CLI**: Wizard milestones M1–M4 plus supporting commands (auth/provider validation/infra/stripe/release/status/secrets) automate environment provisioning, secret rotation, dependency checks, and audit artifact generation under `var/reports`.
...
- **Stripe Evidence Automation**: Extend `starter_cli release db` to capture Stripe CLI transcript + seed artifacts automatically for auditors.
