...
## Success Criteria
- `python -m starter_cli.app start dev --detached` starts infra/backend/frontend, writes PID/log state, and exits 0 after health passes.
- `python -m starter_cli.app stop` stops only tracked PIDs, runs `docker compose down` when infra was ours, and clears state.
...
## Validation Plan
- Run: `hatch run lint`, `hatch run typecheck`, `pytest starter_cli/tests/unit` (focus on new modules), `pnpm lint`, `pnpm type-check` (front-end bindings).
- Manual: `python -m starter_cli.app start dev --detached`, verify badge + doctor stack probe; `python -m starter_cli.app stop`, confirm ports freed and state cleared.

Validation log:
- 2025-11-21 — `python -m pytest starter_cli/tests/unit/home/test_stack_state.py starter_cli/tests/unit/home/test_stop_command.py starter_cli/tests/unit/home/test_start_runner.py` ✅
- 2025-11-21 — `python -m starter_cli.app start dev --detached` ❌ backend exited during migrations (alembic script_location not found) and frontend saw PORT=8000 clash; stack not recorded; cleaned up with `docker compose down`.
