...
What was done
- Walked all SNAPSHOT.md files (root, api-service, web-app, starter_cli, starter_contracts, scripts, ops) and the live tree to spot stray artifacts and path assumptions.
...
- `justfile` delegates: `api_just := "just -f api-service/justfile"`, `cli_just := "just -f starter_cli/justfile"`, `contracts_just := "just -f starter_contracts/justfile"`, `web_just := "just -f web-app/justfile"`; compose_file paths assume `ops/compose/...` from repo root.
- Python type runner `scripts/typecheck.py` hardcodes `api-service/…`, `starter_cli/…`, `starter_contracts/…` paths.
- Sys.path injections targeting `api-service`: `starter_cli/src/starter_cli/__init__.py`; CLI commands (`commands/api.py`), workflows (`workflows/setup/demo_token.py`, `workflows/setup/dev_user.py`); smoke + CI scripts (`scripts/check_secrets.py`, `scripts/smoke/platform_smoke.py`); Alembic/env bootstrap (`api-service/alembic/env.py`, `api-service/src/run.py`).
...
- `starter_cli` → imports backend modules via sys.path injection; also shells to `ops/compose` and reads root `.env*` through `env_runner` in justfile.
...
- Completed moves: `api-service` → `apps/api-service`, `web-app` → `apps/web-app`, `starter_cli` → `packages/starter_cli`, `starter_contracts` → `packages/starter_contracts`, and all former `scripts/` utilities into `tools/` (moduleviz, smoke, assert-billing, vault helpers, typecheck, check_secrets).
...
- Ensure `starter_cli` and `api-service` continue to share contracts only via `starter_contracts` package (no cross-imports through sys.path).
...
- Starter CLI tests now hermetic after storage section addition and env isolation; added prompt_choice support, storage defaults, and regenerated `docs/trackers/CLI_ENV_INVENTORY.md`.
