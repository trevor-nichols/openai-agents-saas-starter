<!-- SECTION: Metadata -->
# Milestone: Textual-First Console Hub

_Last updated: 2025-12-23_  
**Status:** Complete (follow-on hardening tracked in `docs/trackers/CONSOLE_MILESTONE.md`)  
**Owner:** @platform-foundations  
**Domain:** Console  
**ID / Links:** [Docs: starter_console/README.md], [Tracker: CONSOLE_MILESTONE.md]
...
Deliver a Textual-first, interactive console hub that feels native and professional, while preserving non-interactive automation commands. The outcome is a clean, maintainable UI architecture where workflows are UI-agnostic and the console experience reads as “Textual from day one.”
...
- Textual hub is the default entrypoint (`starter-console` with no args).
...
- `hatch run lint` and `hatch run typecheck` pass for `packages/starter_console`.
...
- New features unrelated to console UX redesign.
...
| Docs & runbooks | ✅ | Tracker and console docs updated for Textual-first UX. |
...
- Textual version in `packages/starter_console/pyproject.toml` (already present).
...
| Breaking automation flows | Med | Keep headless presenter, ensure console subcommands remain. |
...
- Manual smoke: `starter-console` (hub), `starter-console setup wizard --non-interactive`, `starter-console logs tail`.
...
See `docs/trackers/CONSOLE_MILESTONE.md` for the active “Textual Hub Hardening (Ports + Hub Service)” milestone. This covers:
- UI ports + presenter adapters (Textual + headless).
- HubService for aggregated pane data.
- Removal of UI → command module dependencies.
- Console subcommands as automation entrypoints for the same workflows.
