...
**Domain:** CLI  
**ID / Links:** [CLI charter], [Stripe CLI workflows], [Textual hub]
...
Enable Stripe provisioning and webhook setup directly inside the Textual hub so developers can complete billing setup end-to-end without leaving the CLI, while preserving clean architecture boundaries and reusing existing Stripe workflows.
...
- `just lint` and `just typecheck` pass for `packages/starter_cli`.
...
- Add CLI unit tests for new runner/logging utilities.
...
- Introduce a Textual workflow session adapter that runs existing CLI workflows in-process and exposes prompt/log channels.
...
- Existing Stripe workflows in `starter_cli/workflows/stripe/*`.
...
- `cd packages/starter_cli && just lint`
- `cd packages/starter_cli && just typecheck`
- `cd packages/starter_cli && just test` (new unit tests)
- Manual: run `python -m starter_cli.app home`, open Stripe pane, run setup + webhook.
