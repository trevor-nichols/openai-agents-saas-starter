...
| STRIPE-02 | Developer Setup Script | `pnpm stripe:setup` verifies Stripe CLI auth, optionally runs `just dev-up`, checks Postgres/Redis, captures secrets into `apps/api-service/.env.local`, and prints next steps; documented in `docs/scripts/stripe-setup.md`. | Completed | The helper now lives under `starter-console stripe setup`, provisioning Starter/Pro products + 7-day-trial prices via the Stripe SDK, recording `STRIPE_PRODUCT_PRICE_MAP`, and writing secrets into `apps/api-service/.env.local`. |
| STRIPE-03 | Gateway Observability | `StripeGateway` emits structured logs + Prom metrics, surfaces domain-specific error codes, and maps plan codes via `stripe_product_price_map`; unit tests cover retries/idempotency. | Completed | Added gateway-level Prom counters/histograms, structured logging + error codes in `app/services/billing/stripe/gateway.py` + `app/services/billing/stripe/telemetry.py`, and metric helpers/tests so Stripe operations are observable end-to-end. |
...
2. **STRIPE-02 – Developer Setup Script**: Deliver the consolidated CLI command (`starter-console stripe setup`, aliased by `pnpm stripe:setup`) so new contributors can authenticate the Stripe CLI, launch Postgres/Redis, auto-provision Starter/Pro prices, and persist secrets into `apps/api-service/.env.local` safely.
...
- **Secrets mishandling**: Setup script must never print raw keys; redact logs and guard `apps/api-service/.env.local` permissions.
...
4. CI has a `just test-stripe` job covering webhook ingestion, replay, and SSE publishing with fixtures.
