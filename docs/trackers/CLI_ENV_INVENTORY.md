...
> Note: the setup wizard also captures `API_BASE_URL` for wiring the Next.js frontend proxy, but
> it is not part of the backend `Settings` model, so it does not appear in this inventory.
