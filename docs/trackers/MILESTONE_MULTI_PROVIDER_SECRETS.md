...
- **CLI:** `starter_cli/cli/common.py` builds Vault envelopes + signatures; `starter_cli/cli/auth_commands.py` depends on Vault headers; setup wizard validators probe the Transit key; `starter_cli/cli/infra_commands.py` shells into Make targets for dev Vault.
...
Located in new module `anything-agents/app/domain/secrets.py` (mirrored under `starter_shared/secrets/models.py`):
```python
