...
| Starter CLI onboarding | ✅ Completed | New "Usage & Entitlements" wizard section toggles guardrails, prompts for plan dimensions, and writes `var/reports/usage-entitlements.json`. |
...
4. **Starter CLI Experience** — New wizard section to select metering dimensions, seed default plan limits, and skip metering entirely if desired; outputs env + plan fixture updates.
...
| P3 — Starter CLI Enablement | Wizard prompts for metering dimensions + default limits, writes `.env` and plan seed updates, supports opt-out. | Headless + interactive flows verified; docs show how to toggle per environment. |
...
| 4 | Add Starter CLI “Usage & Entitlements” wizard section (dimension selection, default plan limits, skip path) + tests. | Platform Foundations | ✅ Completed | Nov 18 |
