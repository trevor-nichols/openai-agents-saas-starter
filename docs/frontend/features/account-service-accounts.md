...
> **Status – 12 Nov 2025:** Automation tab now lists, revokes, and issues tokens. The Issue Token dialog in `/account?tab=automation` calls `/api/v1/auth/service-accounts/browser-issue` (Next `app/api/v1` route), which proxies the FastAPI bridge (`POST /api/v1/auth/service-accounts/browser-issue`) and requires a justification string for auditing.

## 1. Context & Goals

- Route: `app/(app)/(shell)/account/page.tsx` (tab `automation`)
- Panel: `features/account/service-accounts/ServiceAccountsPanel.tsx`
...
- Success signals:
  ...
  - Issuing a token can use browser mode or forward Vault headers; browser mode hits `/api/v1/auth/service-accounts/browser-issue` and vault mode hits `/api/v1/auth/service-accounts/issue`, with the UX still providing the copy-once moment.
...
| ---- | ------ | ----------- | ----- |
| List service accounts | `/api/v1/auth/service-accounts/tokens` (Next `app/api/v1` route) → FastAPI `/api/v1/auth/service-accounts/tokens` | `useServiceAccountTokensQuery` + `DataTable` | Query params support `account`, `status`, pagination; response normalized to camelCase. |
| Revoke token | `/api/v1/auth/service-accounts/tokens/{jti}/revoke` | `useRevokeServiceAccountTokenMutation` | Confirms via AlertDialog + optional reason piped to backend for audits. |
| Issue token | `/api/v1/auth/service-accounts/browser-issue` (session-signed) **or** `/api/v1/auth/service-accounts/issue` (Vault headers) → FastAPI `/api/v1/auth/service-accounts/browser-issue` / `/api/v1/auth/service-accounts/issue` | `useIssueServiceAccountTokenMutation` | Browser mode signs via the logged-in admin session. Vault mode forwards `X-Vault-Authorization` + payload headers captured from Vault Transit/CLI so the canonical issuance endpoint is exercised. |
| Token copy-to-clipboard | Dialog success view | Built-in copy handler | Uses `navigator.clipboard.writeText` and a one-time visibility warning. |

Implementation notes:
- `lib/api/accountServiceAccounts.ts` calls the `/api/v1/...` Next routes so client components never call the FastAPI base URL directly.
...
1. **Listing endpoint** – `GET /api/v1/auth/service-accounts/tokens` returns the token rows used by the UI; responses are normalized to camelCase in the server service layer.
2. **Revoke endpoint** – `POST /api/v1/auth/service-accounts/tokens/{token_id}/revoke` is the canonical revoke path and is used by the UI.
3. **Tenant context** – Issuance always uses the current tenant (derived from the session). Cross-tenant issuance stays in the CLI.
4. **Scopes list** – Scopes are entered as free-form values; keep them aligned with the service-account catalog (`service_accounts.yaml`).
