...
For more elaborate logic pass a callable that receives a `ToolFilterContext`. The callable can be synchronous or asynchronous and returns `True` when the tool should be exposed.
