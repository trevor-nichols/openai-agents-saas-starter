...
1. Run `python -m starter_cli.cli setup wizard --profile <env>` (interactive) or pass `--answers-file` for headless mode.
...
5. Store the resulting `.mmdb` file somewhere persistent (Docker volume, baked artifact). Re-run the wizard or `python -m starter_cli.cli setup wizard --report-only` whenever you rotate the file so the audit log captures the decision.
