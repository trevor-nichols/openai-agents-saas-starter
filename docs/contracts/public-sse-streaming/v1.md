...
**Status:** Implemented (v1; additive evolution pre-release)  
...
- **Agent routing:** agent changes are explicit via `kind="agent.updated"`.
- **MCP approvals:** approval decisions are explicit via `kind="tool.approval"`.
...
### 6.2.1 Scope fields (optional)

Some events are **scoped** to a parent tool call (for example, agent‑as‑tool streaming).
...
- `scope` is omitted for root stream events.
- Scoped events MUST include the same `conversation_id` / `response_id` as the parent stream.
...
### 9.1.1 `memory.checkpoint`

UX marker emitted when the server applies a memory strategy mutation (e.g., compaction)
during a run. This event must **not** change what the user sees in prior messages; it
exists to explain why the model may have lost context.
...
```json
{
  ...
  "checkpoint": {
    ...
    "trigger_reason": "turns | token_budget | token_soft_budget",
    "tokens_before": 12000,
    "tokens_after": 8000,
    "compacted_count": 4
  }
...
Signals that the active agent changed during a run (handoff/routing).
...
### 9.3 `output_item.added` / `output_item.done`
...
Notes:
- For `file_citation` and `container_file_citation`, the server may populate an optional `url` field that points to a first-party download route (relative to the web app origin). Clients may treat this as an opaque download link and should not assume it is a public OpenAI URL.
- For `container_file_citation`, downloads may be authorized using additional query parameters (for example `conversation_id`) embedded in the `url`. Clients should preserve the full URL when linking.

### 9.6 `reasoning_summary.part.added` / `reasoning_summary.part.done`

Reasoning summary parts (chunk boundaries) keyed by `summary_index`.

This is useful for a “bullet per reasoning chunk” UX (like ChatGPT’s Activity view):
- Create a chunk placeholder on `reasoning_summary.part.added`
- Append streamed text via `reasoning_summary.delta` (same `summary_index`)
- Mark the chunk as complete on `reasoning_summary.part.done`
...
```json
{
  ...
  "summary_index": 0,
  "part_type": "summary_text",
  "text": "final summary part text"
}
```

### 9.7 `reasoning_summary.delta`
...
### 9.8 `refusal.delta` / `refusal.done`
...
#### Agent tools (`tool_type="agent"`)
- status: `in_progress | completed | failed`
- fields: `name: string`, `agent?: string`

### 9.10 `tool.approval`

Approval decision for an MCP tool call (distinct from execution lifecycle).
...
### 9.11 `tool.arguments.delta` / `tool.arguments.done`

Streaming tool input for function, agent, and MCP calls.
...
### 9.12 `tool.code.delta` / `tool.code.done`
...
```json
{
  ...
  "item_id": "call_...",
  "tool_call_id": "call_...",
  "tool_type": "web_search | file_search | code_interpreter | image_generation | function | mcp | agent",
  "output": { "any": "json" }
}
```

### 9.14 `chunk.delta` / `chunk.done`
...
Notes:
- `final.attachments` may include server-persisted tool outputs (for example images, or Code Interpreter container files that were copied into tenant storage). Prefer these URLs for durable downloads when present.
...
| `response.reasoning_summary_part.added` / `response.reasoning_summary_part.done` | `reasoning_summary.part.added` / `reasoning_summary.part.done` |
...
| Agents SDK `agent_updated_stream_event` | `agent.updated` |
...
| SDK AgentToolStreamEvent (`on_stream`) | scoped events with `scope.type="agent_tool"` |
| SDK `run_item_stream_event` (`mcp_approval_response`) | `tool.approval` |
...
- `docs/contracts/public-sse-streaming/examples/chat-agent-tool-streaming.ndjson`
- `docs/contracts/public-sse-streaming/examples/chat-image-generation-partials.ndjson`
- `docs/contracts/public-sse-streaming/examples/chat-handoff.ndjson`
