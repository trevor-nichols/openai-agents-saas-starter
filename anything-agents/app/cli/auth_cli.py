...
def build_parser() -> argparse.ArgumentParser:
    ...
    rotate_parser = keys_subparsers.add_parser(
        "rotate",
        help="Generate a new Ed25519 keypair and store it as the next key.",
    )
...
def handle_keys_rotate(args: argparse.Namespace) -> int:
    ...
    status = KeyStatus.ACTIVE if args.activate_now or keyset.active is None else KeyStatus.NEXT

    try:
        material = generate_ed25519_keypair(status=status, kid=args.kid)
        keyset.insert(material)
        keyset.ensure_overlap_within(settings.auth_rotation_overlap_minutes)
        save_keyset(keyset, settings)
    except (KeyStorageError, KeyRotationError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
