...
export default function AgentPage() {
  ...
  const loadMessagesForConversation = useCallback(async (conversationId: string) => {
    ...
    // Placeholder: Reset messages. Implement actual message fetching logic here.
    setMessages([]); 
    setCurrentConversationId(conversationId);
    // Example: Potentially fetch initial messages for this conversation
    // const history = await getConversationHistoryAction(conversationId); // You'd need to create this action
    // if (history.success && history.messages) {
    //   setMessages(history.messages.map(m => ({...m, id: m.timestamp || String(Date.now())}) )); // Adapt to ChatMessage type
    // }
  }, []);
  ...
  const handleSendMessage = async (messageText: string) => {
    ...
    try {
      ...
      if (finalConversationIdFromStream) {
        if (!tempConversationId) { // This was a new conversation
          ...
          const newConvItem: ConversationListItem = {
            ...
            last_message_summary: messageText.substring(0, 50) + (messageText.length > 50 ? "..." : ""),
            updated_at: new Date().toISOString(),
          };
          addConversationToList(newConvItem); 
          ...
          // Conversation ID confirmed, update its timestamp/summary
          const updatedConvItem: ConversationListItem = {
            id: tempConversationId,
            last_message_summary: messageText.substring(0, 50) + (messageText.length > 50 ? "..." : ""),
            updated_at: new Date().toISOString(),
          };
          updateConversationInList(updatedConvItem);
      ...
      setMessages(prevMessages =>
        prevMessages.filter(msg => msg.id !== assistantMessageId)
          .map(msg => msg.id === userMessage.id ? { ...msg, content: `${msg.content} (Error sending)` } : msg)
      );
