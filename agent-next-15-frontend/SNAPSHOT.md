...
│   │   ├── status/                      # API routes backing the marketing status page
│   │   │   ├── route.ts                 # JSON route that proxies the FastAPI platform status snapshot
│   │   │   └── rss/                     # Sub-route for RSS proxying
│   │   │       └── route.ts             # Streams the upstream RSS feed from FastAPI
...
│   │   ├── status.ts                    # API functions for fetching the platform status snapshot via the BFF
...
│   │   ├── status.ts                    # Query hook for retrieving the platform status snapshot
...
│   │   │   ├── status.ts                # Service function fetching the platform status snapshot
...
│   ├── status.ts                        # Types and mappers for platform status responses
