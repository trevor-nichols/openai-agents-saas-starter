...
from starter_contracts.paths import resolve_repo_path
...
def build_rotating_handler(
    ...
    *,
    ...
    max_bytes = int(config.logging_file_max_mb * 1024 * 1024)
    log_root_value = None
    if not use_custom_path:
        log_root_value = str(
            resolve_repo_path(config.log_root or "var/log", repo_root=config.repo_root)
        )

    handler_class = config.file_handler_class
    if not handler_class:
        handler_class = f"{config.handler_namespace}.file.DateRollingFileHandler"
    ...
    return {
        ...
        "max_bytes": max_bytes,
        "backup_count": config.logging_file_backups,
        "logging_max_days": int(config.logging_max_days or 0),
...
def ensure_log_paths(
    ...
    default_path = config.default_file_path or config.logging_file_path
    log_path_value = config.logging_file_path
    use_custom_path = bool(log_path_value) and str(log_path_value) != str(default_path)

    if use_custom_path:
        custom = resolve_repo_path(log_path_value, repo_root=config.repo_root)
        all_path = custom
        ...
        base_root = resolve_repo_path(config.log_root or "var/log", repo_root=config.repo_root)
        today = date.today().isoformat()
        date_root = base_root / today
        component_root = date_root / config.component
        component_root.mkdir(parents=True, exist_ok=True)
        _update_current_symlink(base_root, date_root)
        if config.logging_max_days:
            _maybe_prune_logs(base_root, int(config.logging_max_days))
