...
class OTLPHTTPLogHandler(logging.Handler):
    ...
    def __init__(self, endpoint: str, headers: Mapping[str, str] | None = None) -> None:
        super().__init__()
        if not endpoint:
            raise ValueError("OTLP endpoint is required when LOGGING_SINKS includes otlp")
...
def build_otlp_sink(
    ...
    *,
    ...
    _ = file_selected
    if not config.logging_otlp_endpoint:
        raise ValueError(
            "LOGGING_OTLP_ENDPOINT is required when LOGGING_SINKS includes otlp"
        )
    headers = parse_headers(config.logging_otlp_headers) or {}
    handler_class = config.otlp_handler_class
    if not handler_class:
        handler_class = f"{config.handler_namespace}.otlp.OTLPHTTPLogHandler"
    handlers = {
        "otlp": {
            "class": handler_class,
            "level": log_level,
            "formatter": formatter_ref,
            "endpoint": config.logging_otlp_endpoint,
            "headers": headers,
        }
    }
