...
def build_datadog_sink(
    ...
    *,
    ...
    _ = file_selected
    if not config.logging_datadog_api_key:
        raise ValueError(
            "LOGGING_DATADOG_API_KEY is required when LOGGING_SINKS includes datadog"
        )
    handler_class = config.datadog_handler_class
    if not handler_class:
        handler_class = f"{config.handler_namespace}.datadog.DatadogHTTPLogHandler"
    handlers = {
        "datadog": {
            "class": handler_class,
            "level": log_level,
            ...
            "site": config.logging_datadog_site or "datadoghq.com",
        }
    }
