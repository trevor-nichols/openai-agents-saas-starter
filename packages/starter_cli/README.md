...
- `tools/` is reserved for CI/engineering guardrails (OpenAPI assertions, secret checks, type
  checks, env inventory verification, and Vault dev helpers). User-facing helpers formerly in
  `tools/` have been folded into the CLI.
...
- The exit checklist leaves only two manual steps: `cd apps/api-service && hatch run serve` (backend) and `cd apps/web-app && pnpm dev` (frontend). You can opt to keep Docker Compose running so those commands work immediately; otherwise the wizard tears down infra during cleanup.
