...
from starter_console.core.constants import TRUE_LITERALS
...
PROBE_SPECS: tuple[ProbeSpec, ...] = (
    ...
    ProbeSpec("sso", lambda ctx: _sso_probe(ctx), category="auth", optional=True),
    ProbeSpec("storage", lambda ctx: _storage_probe(ctx), category="storage", optional=True),
)


def _env_probe() -> ProbeResult:
    from starter_console.workflows.home.probes.env import env_coverage_probe

    return env_coverage_probe()


def _ports_probe() -> ProbeResult:
    from starter_console.workflows.home.probes.ports import ports_probe

    return ports_probe()


def _stack_probe() -> ProbeResult:
    from starter_console.workflows.home.probes.stack import stack_probe

    return stack_probe()


def _db_config_probe(ctx: ProbeContext) -> ProbeResult:
    from starter_console.workflows.home.probes.db_config import db_config_probe

    return db_config_probe(ctx)


def _db_probe(warn_only: bool) -> ProbeResult:
    from starter_console.workflows.home.probes.db import db_probe

    return db_probe(warn_only=warn_only)


def _redis_probe(warn_only: bool) -> ProbeResult:
    from starter_console.workflows.home.probes.redis import redis_probe
...
def _migrations_probe() -> ProbeResult:
    from starter_console.workflows.home.probes.migrations import migrations_probe

    return migrations_probe()


def _secrets_probe(ctx: ProbeContext) -> ProbeResult:
    from starter_console.workflows.home.probes.secrets import secrets_probe

    return secrets_probe(ctx)


def _billing_probe(ctx: ProbeContext) -> ProbeResult:
    from starter_console.workflows.home.probes.billing import billing_probe

    return billing_probe(ctx)


def _storage_probe(ctx: ProbeContext) -> ProbeResult:
    from starter_console.workflows.home.probes.storage import storage_probe

    return storage_probe(ctx)


def _sso_probe(ctx: ProbeContext) -> ProbeResult:
    from starter_console.workflows.home.probes.sso import sso_probe

    return sso_probe(ctx)
