...
from starter_console.workflows.secrets import registry as secrets_registry
from starter_console.workflows.secrets.models import OnboardResult
from starter_console.workflows.setup import HeadlessInputProvider
...
from starter_console.workflows.setup._wizard.sections import (
    core,
    frontend,
    secrets,
    security,
...
from starter_console.workflows.setup.answer_recorder import (
    AnswerRecorder,
...
class StubProvider(InputProvider):
    ...
    def prompt_choice(
        ...
        *,
        ...
        return self.strings.get(key, default or (choices[0] if choices else ""))
...
def test_checklist_output_format(cli_ctx: CLIContext) -> None:
    ...
    assert "Starter Console Setup Checklist" in checklist
...
def test_audit_sections_flag_missing_values(
    ...
    monkeypatch.setattr("starter_console.core.context.get_settings", _raise_settings_error)
